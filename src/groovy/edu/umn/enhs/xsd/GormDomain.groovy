...
class GormDomain {
	...
	// TODO: Use the SimpleTemplatingEngine instead
	String generateClassDefinition(Boolean enumsAsDomainClasses) {
		def sb = new StringBuilder()
		def nl = System.getProperty("line.separator")

		sb << "package ${packageName}${nl}${nl}"
		sb << "import javax.xml.stream.XMLStreamException${nl}"
		sb << "import javax.xml.stream.XMLStreamReader${nl}"
		sb << "${nl}"
		...
		// XML Constructor
		sb << "\tpublic ${className}(XMLStreamReader reader, boolean strict) {${nl}"
		sb << "\tif (reader.isStartElement()) {${nl}"
		sb << "\t\tif (reader.getLocalName() == XSD_TABLE_NAME) {${nl}"
		sb << "\t\t\twhile (!reader.isEndElement() ${nl}"
		sb << "\t\t\t\t\t&& reader.getLocalName == XSD_TABLE_NAME ${nl}"
		sb << "\t\t\t\t\t&& reader.hasNext()) {${nl}"
		sb << "\t\t\t\treader.next()${nl}"
		sb << "\t\t\t\tif (reader.startElement && reader.hasText()) {${nl}"
		sb << "\t\t\t\t\tswitch (element.name()) {${nl}"
		properties.each{ p ->
			sb << "\t\t\t\t\t\t\tcase '${p.elementName}':${nl}"
			sb << "\t\t\t\t\t\t\t\t${p.name} = reader.text()${nl}"
			sb << "\t\t\t\t\t\t\t\tbreak${nl}"
		}
		sb << "\t\t\t\t\t\t\tdefault:${nl}"
		sb << "\t\t\t\t\t\t\t\tif (strict) {${nl}"
		sb << "\t\t\t\t\t\t\t\t\tthrow new XMLStreamException(\"malformed XML, unknown element: \${reader.text()}\")${nl}"
		sb << "\t\t\t\t\t\t\t\t}${nl}"
		sb << "\t\t\t\t\t\t\t\tbreak${nl}"
		sb << "\t\t\t\t\t\t}${nl}"
		sb << "\t\t\t\t\t}${nl}"
		sb << "\t\t\t\t}${nl}"
		sb << "\t\t\t} else {${nl}"
		sb << "\t\t\t\tthrow new XMLStreamException(\"xml element did not start as \${XSD_TABLE_NAME}\")${nl}"
		sb << "\t\t\t}${nl}"
		sb << "\t\t} else {${nl}"
		sb << "\t\t\tthrow new XMLStreamException(\"constructor must be passed the start element for the table in the XMLStreamReader\")${nl}"
		sb << "\t\t}${nl}"
		sb << "\t}${nl}"
		sb << "${nl}"
